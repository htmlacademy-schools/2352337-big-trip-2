...
const waypointTemplate = (point, destinations, offersByType) => {
  const destination = destinations.find((dest) => dest.id === point.destination);
  const offersOfType = offersByType.find((offer) => offer.type === point.type).offers;
  const selectedOffers = offersOfType.filter((offer) => point.offers.includes(offer.id));

  return `<li class="trip-events__item">
    <div class="event">
      <time class="event__date" datetime="${formatDate(point.dateFrom)}">${formatDate(point.dateFrom)}</time>
...
export default class WaypointListView {
  ...
  getTemplate() {
    return waypointTemplate(this.points, this.destinations, this.offersByType);
  }
